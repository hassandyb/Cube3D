...
/*   Updated: 2024/01/10 12:11:28 by hed-dyb          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_draw_the_colom(t_prime *prime, int x, int color)
{
	int y;
	int count;
...
void ft_draw_3d_walls(t_prime *prime)
{
	double teta;
	int x ;
	double real;
	...
	while(x < SCREEN_WID)
	{
		...
		real = prime->ray.len * cos(prime->ray.r_angle - prime->player.angle);
		prime->wall.h = (IMG_SIZE * prime->wall.d) / real;

		
		ft_draw_the_colom(prime, x, 0x008000);
